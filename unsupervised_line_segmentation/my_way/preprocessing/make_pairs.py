...
def unsupervised_loaddata(folderName, set_size):
    ...
    for i in range(set_size):
        ...
        p1, p2, label = get_random_pair(folderName)
...
def sample_rotated_patch(img, pos, angle, patch_size):
    ...
    src = np.asarray([[pos[0], pos[1]], [pos[0], pos[1] + patch_size],
                      [pos[0] + patch_size, pos[1] + patch_size], [pos[0] + patch_size, pos[1]]])
    ...
    dst = np.asarray(
        [[0, 0], [0, patch_size - 1], [patch_size - 1, patch_size - 1], [patch_size - 1, 0]],
        dtype=np.float32)
...
def get_rotated_patches(img, thresh):
    ...
    while True:
        ...
        pos = [np.random.randint(2 * x_margin + (i_x + 1) * patch_size,
        ...
        p2_pos = [
            pos[0] + i * np.random.randint(x_min_margin + patch_size, patch_size + x_max_margin),
        ...
        if (cv2.countNonZero(p1_thresh) < 6000) or (cv2.countNonZero(p2_thresh) < 6000):
            continue
        else:
    ...
    if SHOW_RESULTS:
        ...
        cv2.rectangle(disp_img, (int(p1_pos[0]), int(p1_pos[1])),
                      (int(p1_pos[0] + patch_size), int(p1_pos[1] + patch_size)), (255, 255, 0), 5,
                      lineType=cv2.LINE_AA)
        cv2.rectangle(disp_img, (int(p2_pos[0]), int(p2_pos[1])),
                      (int(p2_pos[0] + patch_size), int(p2_pos[1] + patch_size)), (0, 255, 0), 5,
                      lineType=cv2.LINE_AA)
...
def get_backpaired_patches(img, thresh):
    ...
    while True:
        ...
        if (cv2.countNonZero(p1_thresh) < 1000) != (cv2.countNonZero(p2_thresh) < 1000):
            label = 1
            break
        else:
            continue
    ...
    if SHOW_RESULTS:
        ...
        cv2.rectangle(disp_img, (int(p1_pos[0]), int(p1_pos[1])),
                      (int(p1_pos[0] + patch_size), int(p1_pos[1] + patch_size)), (255, 0, 0), 5,
                      lineType=cv2.LINE_AA)
        cv2.rectangle(disp_img, (int(p2_pos[0]), int(p2_pos[1])),
                      (int(p2_pos[0] + patch_size), int(p2_pos[1] + patch_size)), (0, 0, 255), 5,
                      lineType=cv2.LINE_AA)
...
def calculate_patch_average(cc_stats, p_unique_labels):
    ...
    for cc_label in p_unique_labels:
        if cc_stats[cc_label][cv2.CC_STAT_HEIGHT] > max_height or cc_stats[cc_label][
            cv2.CC_STAT_WIDTH] > max_width:
            h_sum += min(cc_stats[cc_label][cv2.CC_STAT_HEIGHT], max_height)
            w_sum += min(cc_stats[cc_label][cv2.CC_STAT_WIDTH], max_width)
            cc_sum += 1
        elif cc_stats[cc_label][cv2.CC_STAT_HEIGHT] < min_height or cc_stats[cc_label][
            cv2.CC_STAT_WIDTH] < min_width:
            continue
        else:
...
def get_different_size_patches(img, thresh):
    ...
    while True:
        ...
        if (min(hw1, hw2) / max(hw1, hw2) < 0.8):  # todo: improve on this condition
            label = 1
            break
        else:
            continue
    ...
    if SHOW_RESULTS:
        ...
        for cc_i in range(1, cc_number):
            x, y, w, h = cc_stats[cc_i][cv2.CC_STAT_LEFT], cc_stats[cc_i][cv2.CC_STAT_TOP], \
                         cc_stats[cc_i][cv2.CC_STAT_WIDTH], cc_stats[cc_i][cv2.CC_STAT_HEIGHT]
            cv2.rectangle(disp_img, (int(x), int(y)), (int(x + w), int(y + h)), (60, 200, 200), 1,
                          lineType=cv2.LINE_AA)
        for cc_i in p1_unique_labels:
            x, y, w, h = cc_stats[cc_i][cv2.CC_STAT_LEFT], cc_stats[cc_i][cv2.CC_STAT_TOP], \
                         cc_stats[cc_i][cv2.CC_STAT_WIDTH], cc_stats[cc_i][cv2.CC_STAT_HEIGHT]
            cv2.rectangle(disp_img, (int(x), int(y)), (int(x + w), int(y + h)), (255, 0, 0), 3,
                          lineType=cv2.LINE_AA)
        for cc_i in p2_unique_labels:
            x, y, w, h = cc_stats[cc_i][cv2.CC_STAT_LEFT], cc_stats[cc_i][cv2.CC_STAT_TOP], \
                         cc_stats[cc_i][cv2.CC_STAT_WIDTH], cc_stats[cc_i][cv2.CC_STAT_HEIGHT]
            cv2.rectangle(disp_img, (int(x), int(y)), (int(x + w), int(y + h)), (0, 0, 255), 3,
                          lineType=cv2.LINE_AA)
        cv2.rectangle(disp_img, (int(p1_pos[0]), int(p1_pos[1])),
                      (int(p1_pos[0] + patch_size), int(p1_pos[1] + patch_size)), (255, 0, 0), 5,
                      lineType=cv2.LINE_AA)
